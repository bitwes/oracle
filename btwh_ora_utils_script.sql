...
----------------------
    ...
    -----------------------------------------------------------------------------
    -----------------------------------------------------------------------------
	FUNCTION get_count_query(in_table IN VARCHAR2, in_col IN varchar2, in_val IN varchar2)RETURN VARCHAR2
	IS
		l_sql VARCHAR2(10000);
	BEGIN
		l_sql := 'select count(1) from ' || in_table || ' where ';
		l_sql := l_sql || in_col || ' like(' || SQ || in_val || SQ || ')';
		RETURN l_sql;
	END;

    -----------------------------------------------------------------------------
    -----------------------------------------------------------------------------
	PROCEDURE print_value_hits(in_col IN varchar2, in_val IN varchar2)
	IS
		l_sql		varchar2(10000);
		l_count		NUMBER;
		l_errors	t_vc2k_table;
	BEGIN
		FOR rec IN (SELECT owner||'.'||table_name AS thing FROM all_tab_cols WHERE column_name like(in_col) ORDER BY thing)LOOP
			l_sql := get_count_query(rec.thing, in_col, in_val);
			--d_print(l_sql);
			...
			begin
		        ...
		        if(l_count > 0)THEN
		        	d_print('['||l_count||']  '||rec.thing||'     '||l_sql);
		        END IF;
            exception
                when others THEN
                	l_errors(l_errors.count + 1) := rec.thing||'::'||sqlerrm;
			END;

		END LOOP;

		FOR i IN 1..l_errors.count LOOP
			dbms_output.put_line(l_errors(i));
		END LOOP;
	END;
...
------------------
    ...
    begin
   	...
   	PROCEDURE print_view_sql(in_view IN varchar2, in_owner varchar2 DEFAULT null)
   	IS
   		l_view_count		NUMBER;
   	...
   	BEGIN
	   	l_view_name := upper(in_view);
	   	SELECT count(1) INTO l_view_count FROM all_views WHERE view_name = l_view_name;
	    SELECT count(1) INTO l_mview_count FROM all_mviews WHERE mview_name = l_view_name;

	   if(l_view_count = 1)THEN
			dbms_output.put_line('-- View '||l_view_name);
			SELECT text INTO l_sql FROM all_views WHERE view_name = l_view_name;
		elsif(l_mview_count = 1)THEN
			dbms_output.put_line('-- Materialized View '||l_view_name);
			SELECT query INTO l_sql FROM all_mviews WHERE mview_name = l_view_name;
		ELSE
			dbms_output.put_line('Could not find view '||l_view_name||' or found too many of them.');
		END IF;
...
BEGIN
	print_value_hits('PART_NUMBER', '44');
END;
